...
MaintenanceService maintenance(phoneLine, batteryMonitor, modem, console, eventLog,
                               clockManager, hardwareControl, powerLogger);
...
bool pendingIncomingCallAction = false;
Preferences resetRecord;
bool resetRecordReady = false;
uint32_t resetRecordId = 0;
uint32_t postResetStartedAt = 0;
bool postResetPending = false;

void updatePostResetSnapshot();
...
void handleHardwareResetCode() {
  clearDialing();
  LogSerial.println("SERVICE: 9999 diagnostic + hardware power cycle");
  eventLog.add("===== HARDWARE RESET 9999 START =====");

  if (modem.isCallActive() || modem.isIncomingCall()) {
    LogSerial.println("SERVICE: 9999 refused because a cellular call is active");
    eventLog.add("9999 hardware reset refused: cellular call active");
    eventLog.add("===== HARDWARE RESET 9999 END =====");
    return;
  }

  // Capture ESP-side state before resetting. These lines are written
  // immediately to LittleFS so they survive the complete power cycle.
  if (resetRecordReady) {
    resetRecordId = resetRecord.getUInt("sequence", 0) + 1;
    resetRecord.putUInt("sequence", resetRecordId);
    eventLog.add("9999 reset record ID: " + String(resetRecordId));
  }
  ...
  if (modem.isCallActive() || modem.isIncomingCall() || modem.ringIndicatorActive()) {
    eventLog.add("9999 reset cancelled: call arrived during diagnostics");
    return;
  }
  if (!resetRecordReady || resetRecord.putUInt("pending", resetRecordId) != sizeof(uint32_t)) {
    eventLog.add("WARNING: post-reset marker unavailable; proceeding with manual reset");
  }
  eventLog.add("HARDWARE RESET: GPIO35 trigger requested; disconnect USB for full power removal");
  LogSerial.println("SERVICE: disconnect USB for a guaranteed full-board reset");
  delay(100);
  hardwareControl.triggerHardwareReset();

  // Normally execution stops when the A4 board removes battery power. Reaching
  // this point means another source, usually USB, kept the ESP32 powered.
  if (resetRecordReady) resetRecord.remove("pending");
  postResetPending = false;
  LogSerial.println("HARD RESET: trigger sent; board remained powered (check USB)");
  eventLog.add("HARDWARE RESET trigger sent; firmware continued, likely because USB was connected");
  eventLog.add("===== HARDWARE RESET 9999 END =====");
...
void setup() {
  ...
  resetRecordReady = resetRecord.begin("resetdiag", false);
  if (resetRecordReady) {
    resetRecordId = resetRecord.getUInt("pending", 0);
    postResetPending = resetRecordId != 0;
    if (postResetPending) {
      eventLog.add("9999 post-reset record pending, ID " + String(resetRecordId) +
                   "; boot reason " + String(static_cast<int>(esp_reset_reason())));
    }
  }
  ...
  postResetStartedAt = millis();
}

void loop() {
  const bool ag1171MustStayAwake =
    phoneState != PhoneState::IDLE || ringGenerator.isActive() ||
    modem.isCallActive() || modem.isIncomingCall();
  hardwareControl.update(ag1171MustStayAwake, phoneLine.isOffHook());
  if (hardwareControl.hookSignalValid()) {
    phoneLine.update();
  }
  ...
  if (modem.takeIncomingCallStarted()) {
    ...
    hardwareControl.requestAg1171Wake();
    pendingIncomingCallAction = true;

    clearDialing();
  }

  // Do not drive RM/F-R or trust SHK until the AG1171's 50 ms wake-up has
  // elapsed. This path is immediate when experimental power saving is off.
  if (pendingIncomingCallAction && hardwareControl.hookSignalValid()) {
    pendingIncomingCallAction = false;
  ...
  updatePostResetSnapshot();
}

// One snapshot per requested manual reset. No automatic recovery is enabled.
void updatePostResetSnapshot() {
  if (!postResetPending || millis() - postResetStartedAt < 30000UL) return;
  if (phoneState != PhoneState::IDLE || phoneLine.isOffHook() ||
      ringGenerator.isActive() || modem.isCallActive() || modem.isIncomingCall() ||
      modem.ringIndicatorActive() || console.atTerminalActive()) return;

  eventLog.add("===== POST-RESET 9999 ID " + String(resetRecordId) + " =====");
  eventLog.add("Uptime: " + String(millis() / 1000UL) + " s");
  if (batteryMonitor.readingValid())
    eventLog.add("Battery: " + String(batteryMonitor.volts(), 3) + " V");
  String post = modem.diagnosticSnapshot("POST-RESET", true);
  eventLog.addMultiline(post);
  if (post.indexOf("[SNAPSHOT INTERRUPTED FOR CALL]") >= 0) {
    eventLog.add("Post-reset snapshot deferred until call ends");
    postResetStartedAt = millis();
    return;
  }
  eventLog.add("===== POST-RESET SNAPSHOT COMPLETE (not a call test) =====");
  postResetPending = false;
  if (resetRecordReady && !resetRecord.remove("pending"))
    eventLog.add("WARNING: could not clear post-reset marker");
}
