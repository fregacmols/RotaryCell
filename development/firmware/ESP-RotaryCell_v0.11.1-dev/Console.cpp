...
void Console::begin() {
  LogSerial.println();
  LogSerial.println(String("ESP-RotaryCell v") + FIRMWARE_VERSION);
...
void Console::handleNormalLine(String line, bool& atTerminalMode, const char* sourceName) {
  ...
  } else if (line == "WIFI" || line == "WIFI ON") {
    _wifiRequested = true;
    LogSerial.println("WIFI: enabling maintenance access point / renewing 10-minute timer");
  } else if (line == "W") {
...
void Console::printHelp() const {
  ...
  LogSerial.println("  WIFI ON  Enable maintenance Wi-Fi for 10 minutes (WIFI also works)");
