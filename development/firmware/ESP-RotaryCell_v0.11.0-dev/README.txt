...
v0.11.0 development hardware controls
-------------------------------------
- GPIO35 is held LOW normally. Dialing 9999 captures the existing pre-reset
  diagnostic snapshot and then sends a 20 ms HIGH pulse to the Audio/Reset A4
  hardware, which should remove LilyGO battery power for approximately four
  seconds. External USB must be disconnected for a guaranteed full reset.
- GPIO21 controls the AG1171 PD input without ever driving it HIGH. LOW requests
  power-down; INPUT/high-impedance releases PD and runs the SLIC.
- The maintenance page reached by dialing 0000 can enable or disable experimental
  AG1171 power saving. The setting always starts OFF after a reboot.
- When enabled and idle/on-hook, the SLIC sleeps for 500 ms, wakes for 60 ms,
  and provides a 20 ms SHK sampling window. Off-hook detection, incoming calls,
  ringing, dialing, and connected calls keep it awake.
- Expected worst-case pickup delay is approximately 580 ms. Both behavior and
  actual energy savings require validation on the assembled RotaryCell.

v0.11.0-dev experimental hardware controls
-------------------------------------------
- Dial 9999 while no cellular call is active to save a pre-reset diagnostic
  snapshot and pulse GPIO35. The Audio/Reset A4 board then removes battery
  power from the LilyGO for approximately four seconds and reconnects it.
- Disconnect both USB ports before using 9999. USB can keep the LilyGO powered
  while the A4 board disconnects the battery path.
- GPIO35 is held LOW normally; a 20 ms HIGH pulse triggers the A4 one-shot.
- The maintenance page reached by dialing 0000 can enable or disable
  experimental AG1171 power saving. It defaults to OFF after every reboot.
- When enabled, GPIO21 pulls AG1171 PD LOW during idle periods. The firmware
  releases GPIO21 to high impedance every 500 ms, waits 60 ms for the SLIC to
  wake, and samples the debounced SHK signal for 20 ms.
- GPIO21 is never driven HIGH, as required by the AG1171 datasheet.
- Incoming calls request an AG1171 wake before physical ringing begins.
- This development build must be bench-tested before becoming public v0.11.0.

Optional INA226 microSD logging
-------------------------------
- `ENABLE_DEVELOPMENT_POWER_LOGGING` in Config.h explicitly controls all INA226
  and microSD development instrumentation. It is enabled in this local test
  build; set it false for a normal/public build.
- When disabled, the firmware does not initialize the INA226 or SD card and the
  development logger panel is completely hidden from the maintenance page.
- When enabled, the R002 (0.002-ohm) INA226 is sampled at 10 Hz and a new
  `POWER###.CSV` file is created on each boot. Writes are buffered and flushed
  every five seconds.
- The maintenance page shows the active filename and latest voltage, current,
  and power only when development logging was explicitly enabled.
- If the INA226 or card is absent, logging disables itself and normal telephone
  operation continues. A FAT32-formatted card is required.
- No additional Arduino libraries are needed; Wire, SPI, SD, and FS are supplied
  by the ESP32 Arduino core.
