...
// Rev A4 / carrier control connections.
constexpr uint8_t PIN_HARD_RESET_TRIGGER = 35;
constexpr uint8_t PIN_AG1171_PD = 21;

// AG1171 PD is active LOW. Running is represented by a high-impedance GPIO;
// the pin must never be driven HIGH. The datasheet specifies a 50 ms wake-up.
constexpr uint32_t AG1171_WAKE_TIME_MS = 60;
constexpr uint32_t AG1171_HOOK_POLL_INTERVAL_MS = 500;
constexpr uint32_t AG1171_HOOK_SAMPLE_WINDOW_MS = 20;
constexpr uint16_t HARD_RESET_TRIGGER_PULSE_MS = 20;

// -----------------------------------------------------------------------------
// Optional development instrumentation
// Set false for a normal/public v0.11.0 build. When false, the INA226, SD card,
// and their maintenance-page status are not initialized or displayed.
// -----------------------------------------------------------------------------
constexpr bool ENABLE_DEVELOPMENT_POWER_LOGGING = true;

constexpr uint8_t PIN_QWIIC_SDA = 3;
constexpr uint8_t PIN_QWIIC_SCL = 2;
...
constexpr uint32_t POWER_LOG_SAMPLE_INTERVAL_MS = 100;
constexpr uint32_t POWER_LOG_FLUSH_INTERVAL_MS = 5000;
...
// -----------------------------------------------------------------------------
constexpr const char* FIRMWARE_VERSION = "0.11.0-dev";
