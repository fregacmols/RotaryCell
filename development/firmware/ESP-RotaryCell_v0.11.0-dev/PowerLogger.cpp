...
void PowerLogger::update() {
  ...
  const float shuntVoltageMv = static_cast<float>(static_cast<int16_t>(rawShunt)) *
                               0.0025f;
  _latestBusVoltageV = busVoltageV;
  _latestCurrentA = currentA;
  _latestPowerW = powerW;
  _hasSample = true;
...
bool PowerLogger::hasSample() const {
  return _hasSample;
}
...
float PowerLogger::latestBusVoltageV() const {
  return _latestBusVoltageV;
}

float PowerLogger::latestCurrentA() const {
  return _latestCurrentA;
}

float PowerLogger::latestPowerW() const {
  return _latestPowerW;
}
