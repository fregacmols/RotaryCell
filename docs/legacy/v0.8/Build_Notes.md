...
The LilyGO external audio amplifier must be enabled. v0.8 sends the required modem GPIO and audio configuration commands in firmware. The key discovery was that calls can be connected while the physical speaker output remains silent unless the amplifier-enable GPIO is asserted.
