...
String Modem::diagnosticSnapshot(const char* label, bool interruptForCall) {
  String report;
  report.reserve(1800);
  ...
  const char* commands[] = {
    "AT", "AT+CGMR", "AT+CIREG?", "AT+CPIN?", "AT+CSQ", "AT+COPS?", "AT+CREG?", "AT+CGREG?",
    "AT+CEREG?", "AT+CLCC", "AT+CGATT?", "AT+CPSI?", "AT+CNMP?",
    "AT+CCLK?"
  };
  for (const char* command : commands) {
    if (interruptForCall && (_incomingCall || _callActive || ringIndicatorActive())) {
      report += "< [SNAPSHOT INTERRUPTED FOR CALL]\n";
      break;
    }
